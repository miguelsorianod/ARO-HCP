...
#EXISTING_CLUSTER_PAYLOAD=$(curl "${FRONTEND_HOST}:${FRONTEND_PORT}${CLUSTER_RESOURCE_ID}?${FRONTEND_API_VERSION_QUERY_PARAM}")
EXISTING_CLUSTER_PAYLOAD=$(cat cluster.json)
}
